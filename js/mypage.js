...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const nicknameEditBtn = document.querySelector(".nickname-edit-btn");
    ...
    try {
      ...
      if (userData.goalName) {
        ...
        if (Array.isArray(userData.goalName)) {
          ...
          goalsToActivate = userData.goalName.map((goal) => goal.trim());
      ...
      console.error("사용자 프로필 로드 실패:", error.response?.data || error);
      alert(
  ...
  nicknameEditBtn.addEventListener("click", async () => {
    if (!isNicknameEditing) {
      ...
      try {
        ...
        console.error("닉네임 저장 실패", error.response?.data || error);
        alert(
          "닉네임 저장에 실패했습니다: " +
            (error.response?.data?.message || error.message)
        );
  ...
  goalsEditBtn.addEventListener("click", async () => {
    if (!isGoalsEditing) {
      ...
      try {
        ...
        console.error("공부 목표 저장 실패", error.response?.data || error);
        alert(
          "공부 목표 저장에 실패했습니다: " +
            (error.response?.data?.message || error.message)
        );
